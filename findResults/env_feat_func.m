...
f(8) = mean(upperEnvPeaks) - mean(lowerEnvPeaks) % bandwidth

f(9) = mean(horzcat(upperEnvPeaks, lowerEnvPeaks)) %offset
...
f(10) = mean(upperEnvValleys) - mean(lowerEnvValleys) % BW w/o uD

f(11) = std(img_matrix) / mean(img_matrix); %normalized std of signal strength
% figure(1);
% imshow(img_matrix)
% hold on
