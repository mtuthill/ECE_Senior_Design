...
for file in nonFallFiles:
	...
	for i in range(3):
		nonFallData.append(numpy.array(eng.binToDct(file, outfile, numFeatures, i)).tolist())
for file in fallFiles:
	print(file)
	outfile = 'out_' + str(int(round(time.time() * 1000))) + '.png'
	for i in range(3):
		fallData.append(numpy.array(eng.binToDct(file, outfile, numFeatures, i)).tolist())
...
print(df)
df.to_csv(r'dataFrameBinary3Slice.csv', index = False, header = True)
